...
layui.define(['layer'], function (exports) {
    ...
    admin.flexible = function (expand) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.flexible(expand);
        }
    ...
    admin.activeNav = function (url) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.activeNav(url);
        }
        if (url) {
    ...
    admin.rollPage = function (d) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.rollPage(d);
        }
    ...
    admin.refresh = function (url) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.refresh(url);
        }
        var $iframe;
        if (!url) {
    ...
    admin.closeThisTabs = function (url) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.closeThisTabs(url);
        }
    ...
    admin.closeOtherTabs = function (url) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.closeOtherTabs(url);
        }
        if (!url) {
    ...
    admin.changeTheme = function (theme, win, noCache, noChild) {
        ...
        if (!noChild) {
            ...
            for (var i = 0; i < ifs.length; i++) {
                admin.changeTheme(theme, ifs[i], true, false);
            }
        }
    };
    ...
    admin.events = {
        ...
        theme: function () {
            ...
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-theme',
                url: option.url || 'page/tpl/tpl-theme.html'
        ...
        note: function () {
            ...
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-note',
                url: option.url || 'page/tpl/tpl-note.html'
            }, admin.parseLayerOption(option)));
        },
        /* 打开消息 */
        message: function () {
        ...
        lockScreen: function () {
            var url = $(this).data('url');
            admin.strToWin($(this).data('window')).layui.admin.lockScreen(url);
        }
    };
    ...
    admin.lockScreen = function (url) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.lockScreen(url);
        }
    ...
    admin.unlockScreen = function (isRemove) {
        if (window !== top && !admin.isTop() && top.layui && top.layui.admin) {
            return top.layui.admin.unlockScreen(isRemove);
        }
        var $lock = $('#ew-lock-screen-group');
    ...
    admin.recoverState = function () {
        ...
        // 恢复tab自动刷新
        if (setter.pageTabs && setter.tabAutoRefresh) $(tabDOM).attr('lay-autoRefresh', 'true');
    };
        ...
        if (admin.getPageWidth() > 768) {
            ...
            if ($navChild.length > 0) {
                ...
                layer.tips(tipText, $that, {
                    tips: [2, '#303133'], time: -1, success: function (layero) {
    ...
    $(document).on('click', '*[ew-href]', function () {
        ...
        if (win.layui && win.layui.index) {
            win.layui.index.openTab({
                title: title ? title : '',
                url: url
            });
        } else {
    ...
    /* ios浏览器iframe兼容 */
    if (layui.device().ios) $('body').addClass('ios-iframe-body');
    ...
    if (cache) {
        ...
        for (var i = 0; i < keys.length; i++) {
            if (cache[keys[i]] !== undefined) setter[keys[i]] = cache[keys[i]];
        }
    }
    admin.recoverState();
    admin.setter = setter;
