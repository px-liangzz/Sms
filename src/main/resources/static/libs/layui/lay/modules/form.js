      ...
      select: function(){
        ...
        ,events = function(reElem, disabled, isSearch){
          ...
          dds.on('click', function(){
            ...
            select.trigger('change');  // TODO EasyWeb下拉选择触发change事件
    ...
    layui.each(verifyElem, function(_, item){
      ...
      layui.each(vers, function(_, thisVer){
        ...
        if(verify[thisVer]){
          ...
          if(isTrue || isTrue === '' ) {
            if(errorText) {  // TODO EasyWeb 增加表单验证返回空则不处理提示
              //提示层风格
              if(verType === 'tips'){
                // TODO EasyWeb 增加表单验证tips可自定义背景颜色
                layer.tips(errorText, function () {
                  ...
                  return othis;
                }(), {tips: [othis.attr('lay-direction') || 3, othis.attr('lay-bg') || '#ff4c4c']});
                /*layer.tips(errorText, function(){
                  if(typeof othis.attr('lay-ignore') !== 'string'){
                    if(item.tagName.toLowerCase() === 'select' || /^checkbox|radio$/.test(item.type)){
                      return othis.next();
